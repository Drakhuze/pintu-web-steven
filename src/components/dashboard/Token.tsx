...
function Token({ row }: Props) {
  ...
  const [priceColor, setPriceColor] = useState('text-black dark:text-white');
  const [lowPriceColor, setLowPriceColor] = useState('text-black dark:text-white');
  const [highPriceColor, setHighPriceColor] = useState('text-black dark:text-white');
  ...
  const percentColor = Number(row.priceChangePercent) === 0 ? 'text-black dark:text-white' : Number(row.priceChangePercent) > 0 ? 'text-green-500' : 'text-red-500';
  ...
  useLayoutEffect(() => {
    ...
    if (Number(price) !== Number(row.price)) {
      ...
      setTimeout(() => {
        setPriceColor(`text-black dark:text-white ${transition}`);
      }, 500);
    }
    if (Number(lowPrice) !== Number(row.lowPrice)) {
      ...
      setTimeout(() => {
        setLowPriceColor(`text-black dark:text-white ${transition}`);
      }, 500);
    }
    if (Number(highPrice) !== Number(row.highPrice)) {
      ...
      setTimeout(() => {
        setHighPriceColor(`text-black dark:text-white ${transition}`);
      }, 500);
    }
  }, [highPrice, lowPrice, price, row]);

  return (
    <>
      <tr key={row.name} className="token token-lg hidden lg:table-row hover:bg-slate-100 dark:hover:bg-slate-900 cursor-pointer">
        <td className="">
          <div
            className="h-10 w-10 rounded-full mx-auto border dark:border-neutral-800"
            style={{
